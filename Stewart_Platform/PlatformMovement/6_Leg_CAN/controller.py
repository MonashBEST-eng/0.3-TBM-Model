...
# ========================= CONSTANTS =========================
NUM_LEGS_SINGLE = 2  # Reduced for single ECU testing
NUM_LEGS_MULTIPLE = 6 # Used for Multi ECU testing
...
def computeLegs(d_p, phi, theta, psi):
    ...
    l_des = np.zeros(NUM_LEGS_SINGLE)

    for i in range(NUM_LEGS_SINGLE):
        p = np.array([
...
class StewartController:
    ...
    def can_send_prepare(self, d1, t1_ms, d2, t2_ms):
        ...
        # send_can_frame(0x200, data)
        # send_can_frame(0x300, data)
    ...
    def can_process(self, vals, t_max):
        # Preparing First ECU
        data = self.can_send_prepare(vals[0], vals[1], vals[2], vals[3])
        send_can_frame(0x100, data)
        ...
        # Start moving Actuators
        send_can_frame(0x100, bytes([0xFF]))
        send_can_frame(0x200, bytes([0xFF]))
        send_can_frame(0x300, bytes([0xFF]))
        print("Started Actuator Movement")
    ...
    def log_movement(t_max, dirs, times_s, prev_l, l_des):
        ...
        if t_max > 0:
            with open("stewart_trajectory_single.txt", "a") as fp:
                ...
                fp.write("t(s)")
                for i in range(1, NUM_LEGS_SINGLE + 1):
                    fp.write(f"\tL{i}(m)")
                fp.write("\n")
    ...
    def send_bangbang_move(self, l_des):
        ...
        # Starts the process of Actuator Movement
        self.can_process([d1, t1, d2, t2], t_max)

        # Update current position
        prev_l = self.l_curr.copy()
        self.l_curr[:] = l_des[:]

        self.log_movement(t_max, dirs, times_s, prev_l, l_des)