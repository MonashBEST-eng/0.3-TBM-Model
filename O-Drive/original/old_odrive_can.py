...
def wait_for_axis_states(node_ids, target_state, timeout=CLOSED_LOOP_TIMEOUT):
    ...
    for every node in node_ids, or raise TimeoutError. This is what actually
    lets you start both axes together, instead of guessing with a fixed sleep.
    """
...
def run_velocity_loop(velocity):
    """Fixed-rate loop: send both axes' setpoints back-to-back, then sleep
    only the remaining time in the period so the two axes never drift
    relative to each other or to wall-clock time."""
    next_tick = time.perf_counter()
    while True:
        send_vel(AXIS0_NODE_ID, velocity)
        send_vel(AXIS1_NODE_ID, velocity)
