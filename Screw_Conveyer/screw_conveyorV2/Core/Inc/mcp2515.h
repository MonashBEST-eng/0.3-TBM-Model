 ...
 *
 * Wiring assumed (STM32G431RB, NUCLEO-G431RB, SPI1):
 *   SCK  -> PA5  (D13)
 *   MISO -> PA6  (D12)
 *   MOSI -> PA7  (D11)
 *   CS   -> PC7  (D9)   — software-controlled GPIO output, labeled MCP2515_CS in CubeMX
 *   INT  -> PA9  (D8)   — configured but unused (driver polls instead), labeled MCP2515_INT in CubeMX
 *
...
/* DIAGNOSTIC: returns the HAL_StatusTypeDef of the most recent SPI
 * transaction (HAL_OK / HAL_ERROR / HAL_TIMEOUT). Check this after a
 * suspicious register read (e.g. an unexpected 0x00) to tell apart "the
 * chip genuinely responded with this value" from "the SPI transaction
 * itself never completed at all". */
