...
class StewartController:
    def __init__(self):
        # Auto-detect 3 STM32 VCP ports or ask manually
        ports = [p.device for p in serial.tools.list_ports.comports()
                 if "USB" in p.description or "STM" in p.description or "ACM" in p.device]

        if len(ports) >= 3:
            self.serials = [serial.Serial(p, BAUDRATE, timeout=1) for p in ports[:3]]
            print("Auto-detected ports:", [p for p in ports[:3]])
        else:
            print("Could not auto-detect 3 STM32 ports. Enter them manually.")
            self.serials = []
            for i in range(3):
                p = input(f"  ECU{i+1} COM port (e.g. COM4): ").strip()
    ...
    def send_bangbang_move(self, l_des):
        ...
        if not all(prepared):
            print("ERROR: Not all ECUs responded PREPARED")
            return

        # === Step 2: START all ECUs (almost simultaneous) ===
        for ser in self.serials:
            ser.write(b"START\n")

        print(">>> All 6 actuators started (full speed bang-bang) <<<\n")

        # === Step 3: Wait for all DONE ===
        done = [False] * 3
        ...
        # Update current position (exact - no drift)
        self.l_curr[:] = l_des[:]

        # === Log the actual hardware trajectory (bang-bang) ===
        if t_max > 0:
            with open("stewart_trajectory.txt", "a") as fp:
                fp.write("--- Bang-bang trajectory (hardware) ---\n")
                fp.write("t(s)")
                ...
                for step in range(n_steps):
                    ...
                    for i in range(NUM_LEGS):
                        ...
                        line += f"\t{pos:.6f}"
                    fp.write(line + "\n")
            print("Trajectory logged to stewart_trajectory.txt\n")
...
def main():
    ...
    while True:
        ...
        if cmd == 'Q':
            ...
            try:
                vals = list(map(float, input("Enter 6 actuator lengths (m) separated by space: ").split()))
                if len(vals) != 6:
                    raise ValueError
                l_des = np.array(vals)
            ...
            try:
                ...
                print("Computed actuator lengths:")
                for i in range(6):
                    print(f"L{i+1} = {l_des[i]:.4f} m", end="  ")
                print("\n")
